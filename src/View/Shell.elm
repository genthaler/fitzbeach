...
type alias Config msg =
    ...
    , content : Theme.Palette -> Bool -> Element msg
    }
...
view config =
    let
        ...
        colors =
            Theme.palette config.themeMode

        compactLayout : Bool
        compactLayout =
            isCompact config.viewport.width
    ...
    layout
        ...
        (el
            ...
            (column
                ...
                [ siteHeader compactLayout colors config.currentPage config.themeMode config
                , config.content colors compactLayout
                ]
            )
        )
